 ...
 * vector when the entire vector is being freed (using VectorDispose).  This
 * function is your chance to do any deallocation/cleanup required for the
 * element (such as freeing/deleting any pointers contained in the element). The
 * client can pass NULL for the ArrayFreeFunction if the elements don't require
 * any special handling.
 *
 * The initialAllocation parameter specifies the initial allocated length
 * of the vector, as well as the dynamic reallocation increment for those times
 * when the vector needs to grow.  Rather than growing the vector one element at
 * a time as elements are added (inefficient), you will grow the vector in
 * chunks of initialAllocation size.  The allocated length is the number of
 * elements for which space has been allocated: the logical length is the number
 * of those slots currently being used.
 *
 ...
 * so there is little point in asking.  Just leave the vector over-allocated and
 * no one will care.
 *
 ...
 * initialAllocation size.  You want to minimize the number of reallocations,
 * but you don't want to pre-allocate all that much memory if you don't expect
 * to use very much of it.  If the client passes 0 for initialAllocation, the
 * implementation will use the default value of its own choosing.  As assert is
 * raised is the initialAllocation value is less than 0.
 */

void VectorNew(vector *v, int elemSize, VectorFreeFunction freefn,
 ...
 * -----------------------
 * Frees up all the memory of the specified vector and its elements.  It does
 * *not* automatically free memory owned by pointers embedded in the elements.
 ...
 * becomes invalid after any calls which involve insertion into, deletion from
 * or sorting of the vector, as all of these may rearrange the elements to some
 * extent.
 */
 ...
 * ----------------------
 * Inserts a new element into the specified vector, placing it at the specified
 * position. An assert is raised if n is less than 0 or greater than the logical
 * length. The vector elements after the supplied position will be shifted over
 * to make room. The element is passed by address: The new element's contents
 * are copied from the memory pointed to by elemAddr.  This method runs in
 * linear time.
 */
 ...
 * passed by address, and the element contents are copied from the memory
 * pointed to by elemAddr.  Note that right after this call, the new element
 * will be the last in the vector; i.e. its element number will be the logical
 * length minus 1.  This method must run in constant time (neglecting the memory
 * reallocation time which may be required occasionally).
 */
 ...
 * being overwritten, the VectorFreeFunction that was supplied to VectorNew is
 * levied against the old element.  Then that position in the vector will get a
 * new value by copying the new element's contents from the memory pointed to by
 * elemAddr. An assert is raised if n is less than 0 or greater than the logical
 * length minus one.  None of the other elements are affected or rearranged by
 * this operation, and the size of the vector remains constant. This method must
 ...
 *
 * An assert is raised if position is less than 0 or greater than the logical
 * length minus one.  All the elements after the specified position will be
 * shifted over to fill the gap.  This method runs in linear time.  It does not
 * shrink the allocated size of the vector when an element is deleted; the
 * vector just stays over-allocated.
 */
 ...
 * to specify that the vector is already in sorted order, in which case
 * VectorSearch uses a faster binary search.  If isSorted is false, a simple
 * linear search is used.  If a match is found, the position of the matching
 * element is returned; else the function returns -1.  Calling this function
 * does not re-arrange or change contents of the vector or modify the key in any
 * way.
 *
...
int VectorSearch(const vector *v, const void *key,
                 VectorCompareFunction searchfn, int startIndex, mybool isSorted);
 ...
 * element n-1, inclusive) and calls the function mapfn on each element.  The
 * function is called with the address of the vector element and the auxData
 * pointer. The auxData value allows the client to pass extra state information
 * to the client-supplied function, if necessary.  If no client data is
 * required, this argument should be NULL.  An assert is raised if the mapfn
 * function is NULL.
 */
