 ...
 * return value semantics as the strcmp library function (negative if A<B, zero
 * if A==B, positive if A>B).
 */
...
static void InsertPermutationOfNumbers(vector *numbers, long n, long d) {
  ...
  long residue;
  fprintf(stdout,
          "Generating all of the numbers between 0 and %ld (using some number "
          "theory). ",
          d - 1);
...
static void DeleteEverythingVerySlowly(vector *numbers) {
  ...
  fprintf(stdout, "Erasing everything in the vector by repeatedly deleting the "
                  "100th-to-last remaining element (be patient).\n");
  fflush(stdout);
...
static void MemoryTest() {
  ...
  const int kNumQuestionWords =
      sizeof(kQuestionWords) / sizeof(kQuestionWords[0]);
  ...
  for (i = 0; i < kNumQuestionWords; i++) {
    ...
    VectorInsert(&questionWords, &questionWord,
                 0); // why the ampersand? isn't questionWord already a pointer?
  }

  fprintf(stdout, "Mapping over the char * vector (ask yourself: why are char "
                  "**'s passed to PrintString?!!)\n");
